...
**Update (2026-09-03, later the same day) — re-investigated during the UI wow-factor redesign, correcting the note above.** Using an exact `getByRole('button', { name: 'Copy' })` locator (confirmed to match exactly one element), granting clipboard permissions explicitly, and polling the button's `outerHTML` from 50ms out to 5000ms after the click (well past the component's own 2500ms auto-reset timer): the underlying action is genuinely correct — `navigator.clipboard.writeText()` resolves in ~20ms and `navigator.clipboard.readText()` afterward returns the exact right address every time — but the button's rendered text never changes to "Copied!" at any sampled point in headless Chromium via Playwright. To rule out a regression from this session's redesign work, the exact same test was run against a clean worktree of the pre-session baseline commit (`f5dd429`, before any of this iteration's changes): identical result. So this is not something introduced by this pass, and the earlier "confirms it works correctly" claim in the note above does not hold up under a more rigorous, longer-duration re-test — it was very likely itself a fast/shallow check that didn't wait long enough or verify the DOM text directly. What's actually established: the copy action itself is reliably correct (verify via `navigator.clipboard.readText()`, not the button label) under automation; whether real users in a real, focused, non-automated browser tab see the "Copied!" text update could not be determined in this environment (no headed-browser display available here). Future reviews: treat clipboard content as the authoritative pass/fail signal for this feature, not the button's DOM text, and do not claim the visible confirmation "works" without an extended (2s+) poll — a quick single check is not sufficient either direction.
